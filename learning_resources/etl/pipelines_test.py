...
def test_professional_ed_etl(mocker):
    """Verify that the professional education etl pipeline executes correctly"""
    mocker.patch("learning_resources.etl.professional_ed.extract")
    mock_transform = mocker.patch(
        "learning_resources.etl.professional_ed.transform",
        return_value=(
            [{"a": "b"}, {"c": "d"}],
            [{"e": "f"}, {"g": "h"}],
        ),
    ...
    results = pipelines.professional_ed_etl()
    mock_load_courses.assert_called_once_with(

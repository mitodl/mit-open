...
def extracted_and_transformed_values(youtube_api_responses):
    ...
    extracted = [
        ...
        (
            ...
            [
                (playlists_list[2]["items"][0], csail_videos1),
                (playlists_list[3]["items"][1], csail_videos2),
            ],
        ),
    ]
...
def test_get_captions_for_video(mocker):
    ...
    mocker.patch(
        "learning_resources.etl.youtube.pytube.YouTube",
        return_value=Mock(captions=captions_list),
    )
...
def test_get_captions_for_video_autogenerated_only(mocker):
    ...
    mock_pytube_client = mocker.patch(
        "learning_resources.etl.youtube.pytube.YouTube",
        return_value=Mock(captions=captions_list),
    )
    ...
    mock_pytube_client.assert_called_once()
    assert captions == "English: Auto-generated"
...
def test_get_captions_for_video_no_english_caption(mocker):
    ...
    mock_pytube_client = mocker.patch("learning_resources.etl.youtube.pytube.YouTube")
    mock_pytube_client.return_value.captions = captions_list
...
def test_extract_with_exception(
    ...
    playlist_data_mitx, videos_mitx = playlists_mitx[0]
    ...
    playlist_data_csail1, videos_csail1 = playlists_csail[0]
    playlist_data_csail2, videos_csail2 = playlists_csail[1]
...
def test_get_youtube_transcripts(mocker):
    ...
    mock_caption = Mock()
    mock_resource = Mock(id=1, video=Mock())
    ...
    mock_parse_call = mocker.patch(
        "learning_resources.etl.youtube.parse_video_captions"
    )
    ...
    mock_resource.video.save.assert_called_once()
...
def test_get_youtube_transcripts_with_multiple_consecutive_failures(mocker):
    ...
    mock_resource = Mock(id=1, video=Mock())
    video_list = [mock_resource for _ in range(30)]
    ...
    mock_resource.video.save.assert_not_called()
